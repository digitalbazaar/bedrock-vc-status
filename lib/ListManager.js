...
// FIXME: enable configuring limits on the maximum number of lists -- and
...
// FIXME: add another layer to determine SL ID and SL allocation; move
// the code that randomly generates an SL ID behind this API as one possible
// implementation; other implementations may have a limited number of SLs with
// deterministic SL IDs
...
// FIXME: rework the above to separate status tracking from VC issuance more,
// i.e., define status tracking in terms of APIs that a VC issuer can call
// and allow for optimistic index (pre-)allocation if possible:
// FIXME: statusService.requestIndexes({count: 10}) =>
//   status list URL + indexes
// FIXME: statusService.claimIndex({credentialId, index, on/off?}) =>
//   true or conflict exception if index already claimed
...
export class ListManager {
  ...
  async _addCapacity({target}) {
    ...
    // FIXME: concurrently, this variable actually limits the `need` below
    // instead of being a cap on the most active lists, this needs adjustment
    // and/or augmentation and an exception needs to be thrown if more capacity
    ...
    while(activeListSize < target) {
      ...
      for(const x of newActive) {
        // FIXME: with new SLC allocation API, an exception could be thrown
        // here based on total SLC capacity having been reached... which
        // should result in need being unable to be met ... but that doesn't
        // mean some of the need can't be met (check if need==1 before
        // re-throwing an exception on this basis)
  ...
  // FIXME: rename to `getNextSlcId()` and call an internal provider to
  // provide this; require the contract for that API to indicate that the
  // returned value must be unique across concurrent requests (which could
  // take some significant overhead to ensure) or find a mechanism that will
  // avoid / integrate those IDs to prevent misallocating the same SLC ID
  // in multiple LMDs (this assumption was previously achieved with always
  // using pseudo-random SLC IDs)... also need to ensure SLC IDs aren't wasted
  // if they aren't a free resource
  async _createSlcId() {
    // FIXME: enable each SLC ID to provided, optionally, from a
    // configuration instead of being generated here; allow the config in
    // this case to have a limited number of SLCs that can be used and prevent
    // overflow, throwing errors once every list index has been allocated
