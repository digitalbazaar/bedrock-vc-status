...
1. If a LS has been assigned to the writer instance (then a duplicate
...
export class CredentialStatusWriter {
  ...
  // FIXME: if this writer runs on the status service, then there is no
  // guarantee which writer will be interacted with by an issuer service;
  // FIXME: issuer service (client) => get index ... which triggers write()
  // ... a handle to the same writer would be needed to properly call finish();
  // ... this could perhaps be done automatically if requesting an index for a
  // ... VC is changed to be immutable within the status service storage vs.
  // ... relying on the VC storage system to enforce / track with unique index
  async write({credential} = {}) {
    ...
    if(listShard) {
      // FIXME: just pass the index to claim instead of the whole credential
      // as part of separating the status service from the credential service
      ...
      if(doc.content.slSequence === slSequence &&
        ...
        // FIXME: revisit whether this assumption is tenable in more decoupled
        // issuer/status services -- and whether an additional call needs to
        // be made to store the credential ID => SLC ID+index unique mapping
        // along with the status tracking information instead -- this could be
        // done concurrently with storing a VC, but if it fails, then errors
        // or potentially transactional unwinding has to be dealt with

        /* Note: This step works because the index allocation tracking assumes
        that the issuance system will prevent duplicate status indexes from
        being issued for the same SLC. This is externalized from this system,
        which optimizes away an additional write to track this information that
        would otherwise have to be awaited (and potential conflicts dealt with,
        etc.). */
  ...
  // FIXME: this information should be returned from the writer instead to
  // enable separation of the issuer service from the status service
  _upsertStatusEntry({
