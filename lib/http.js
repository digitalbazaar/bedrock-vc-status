...
export async function addRoutes({app, service} = {}) {
  ...
  const routes = {
    ...
    terseSlcs: `${baseUrl}${cfg.routes.terseSlcs}`
  };
  ...
  app.post(
    ...
    asyncHandler(async (req, res) => {
      ...
      try {
        ...
        const {id, type, /*indexAllocator,*/ length, statusPurpose} = req.body;
        // FIXME: validate `id` against `config.id` and SLCs route
        await slcs.create({config, id, type, statusPurpose, length});
        res.status(204).location(id).send();
      } catch(error) {
  ...
  app.options(routes.terseSlcs, cors());
  app.post(
    routes.terseSlcs,
    cors(),
    validate({bodySchema: createStatusListBody}),
    ...
    asyncHandler(async (req, res) => {
      try {
        ...
        await slcs.setStatus({
          config, id: credentialId, credentialStatus, status: true
        });
  ...
  app.get(
    ...
    asyncHandler(async (req, res) => {
      ...
      const {slcId} = req.params;
      const id = `${config.id}${cfg.routes.slcs}/${encodeURIComponent(slcId)}`;
      const {credential} = await slcs.getFresh({config, id});
      res.json(credential);
    }));
