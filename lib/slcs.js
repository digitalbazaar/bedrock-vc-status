...
import {assertSlcDoc, getDocumentStore} from './helpers.js';
...
bedrock.events.on('bedrock-mongodb.ready', async () => {
  ...
  // FIXME: add VC => SLC mapping collection, determine if the index needs
  // to include the status instance config ID or if it's safe to omit that
  // in the face of conflicts across them
});
...
export async function refresh({id, config} = {}) {
  ...
  // FIXME: use `get()` instead
  const [documentStore] = await Promise.all([
  ...
  while(true) {
    try {
      ...
      delete slc.proof;
      slcDoc.content = await issue({config, credential: slc});
...
export async function setStatus({id, config, credentialStatus, status} = {}) {
  ...
  // FIXME: will need another collection to store credential ID => SLC mappings
...
export function _getMatchingStatusListCredentialMeta({
  ...
  throw new BedrockError(
    ...
    'is not supported by this status instance.', 'NotSupportedError', {
      httpStatusCode: 400,
      public: true
    });
}
