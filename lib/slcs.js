...
bedrock.events.on('bedrock-mongodb.ready', async () => {
  ...
  await database.createIndexes([{
    ...
    // FIXME: index needs to include the status instance config ID to prevent
...
export async function setStatus({id, config, credentialStatus, status} = {}) {
  ...
  let record = await get({id: statusListCredential, useCache: false});
  ...
  while(true) {
    try {
      ...
      record = await get({id: statusListCredential, useCache: false});
    }
  }
}
